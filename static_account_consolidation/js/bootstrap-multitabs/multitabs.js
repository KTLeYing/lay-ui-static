...
((function ($) {
    ...
    defaultLayoutTemplates = {
        ...
        default: '<div class="mt-wrapper {mainClass}" style="height: 100%;" >' +
            ...
            '<li class="lyear-aside-toggler"><a><i class="mdi mdi-menu"></i></a></li>' +
            '</ul>' +
            '</div>' +
            ...
            '        <span id="account-name">anonymous<span class="caret"></span></span>\n' +
            '    </a>\n' +
            '    <ul class="dropdown-menu dropdown-menu-right">\n' +
            // '<li><a href="javascript:void(0)" id="key_binder_btn"><i class="mdi mdi-key"></i> bind key</a>\n' +
            '        <li class="divider"></li>\n' +
    ...
    MultiTabs.prototype = {
        ...
        active: function (navTab) {
            ...
            setHash(navTabParam.url, navTabParam?.title?.trim())
            return self;
        },
        /**
         * fill tab pane
        ...
        _init: function (options) {
            ...
            $el.navPanel.css('width', 'calc(100% - 220px)');
        ...
        _final: function () {
            ...
            if ($.isEmptyObject(storage)) {
                init = (!$.isEmptyObject(init) && init instanceof Array) ? init : _defaultInit;
            ...
            if (!$el.navPanelList.children('li.active').length) {
                self.active($el.navPanelList.find(':first'));
            }
            return self;
        },
...
function setHash(link, name) {
    if(link==null){
        window.location.hash =""
    }else{
        window.location.hash = encodeURI(link) + '{' + name + '}'
    }
}
function getHashUrl() {
    return decodeURI(window.location.hash).substring(1).replace(/{.*}$/, "")
}
function getHashName() {
    return decodeURI(window.location.hash).replace(/^.*{(.*)}$/, "$1")
}